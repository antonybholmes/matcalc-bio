...
public class PermuteDialog extends ModernDialogTaskWindow {
	...
	private ModernCompactSpinner mTextSize1 = 
			new ModernCompactSpinner(1, 1000, 10);
	
	private ModernCompactSpinner mTextSize2 = 
			new ModernCompactSpinner(1, 1000, 10);
	
	private ModernTextField mTextPrefix = 
			new ModernTextField("cls_perm");
	
	private ModernCompactSpinner mTextPermutations = 
			new ModernCompactSpinner(1,100000, 10);
	
	private ModernTextField mTextLocation = new ModernTextField();

	private ModernCheckBox mCheckSampleWithReplacement =
			new ModernCheckBox("Sample with replacement");

	private ModernButtonWidget mChangeButton = new ModernDialogFlatButton("Change...", 
			UIService.getInstance().loadIcon("open", 16));
	
	private Path mDir = null;

	public PermuteDialog(ModernWindow parent,
	...
	private void setup() {
		...
		mTextPrefix.setText(TextUtils.replaceSpaces(mGroups.get(0).getName().trim()) + 
				"_vs_" + 
				TextUtils.replaceSpaces(mGroups.get(1).getName().trim()) +
				"_perm");
	...
	private static XYSeries getGroup(XYSeriesGroup groups, ModernComboBox groupCombo) {
