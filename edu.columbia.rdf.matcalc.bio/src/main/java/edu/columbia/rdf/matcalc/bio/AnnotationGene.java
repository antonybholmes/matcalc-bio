...
public class AnnotationGene implements Comparable<AnnotationGene>, TextIdProperty {
	...
	private static final Pattern SYMBOL_REGEX =
			Pattern.compile("gene_name=\"(.+?)\"");

	private static final Pattern TRANSCRIPT_REGEX =
			Pattern.compile("transcript_id=\"(.+?)\"");
	...
	public void addAltName(String type, String name) {
		...
		mText = mId + " " + TextUtils.parenthesis(Join.
				onComma().
				values(CollectionUtils.toList(mAltMap)));
	}
	...
	public String getEntrez() {
		if (mEntrezType == null) {
			mEntrezType = CollectionUtils.contains(mAltMap, 
					"Entrez Id", 
					"entrez_id", 
					"entrez", 
					"Entrez");
	...
	/**
	 * Test for multiple alternative names returning the first found or
	 * n/a if nothing matches.
	 * 
	 * @param names
	 * @return
	 */
	...
	/*
	public static FixedGapSearch<AnnotationGene> parseTssForFixedGappedSearch(Path file, 
			int ext5p, 
			int ext3p) throws IOException {

		FixedGapSearch<AnnotationGene> search = 
				new FixedGapSearch<AnnotationGene>(10000);
	...
	public static FixedGapSearch<AnnotationGene> parseTssForSearch(String name,
			...
			int ext3p) throws IOException {
		FixedGapSearch<AnnotationGene> search = 
				new FixedGapSearch<AnnotationGene>();

		parseTssForSearch(file, 
				ext5p, 
				ext3p,
				search);
	...
	public static BinarySearch<AnnotationGene> parseTssForBinarySearch(String name,
			Path file, 
			int ext5p, 
			int ext3p) throws IOException {

		BinarySearch<AnnotationGene> search = 
				new BinarySearch<AnnotationGene>();
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				List<Integer> exonStarts =
						TextUtils.toInt(exonSplitter.text(tokens.get(EXON_START_COL)));

				List<Integer> exonEnds =
						TextUtils.toInt(exonSplitter.text(tokens.get(EXON_END_COL)));
